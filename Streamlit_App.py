...
try:
  ...
  if not fruit_choice:
    ...
    # Print the  output  as a frame (table)
...
# New text  Input  to allow the end user to add a fruit to the  list
...
# -----------------------------------
streamlit.header("You  can add another Fruit")
...
# Add a Button to add the Fruits
if streamlit.button('Add the fruit to the Fruit Load List'):
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
